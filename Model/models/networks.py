...
def get_norm_layer(norm_type='instance'):
    ...
    if norm_type == 'batch':
        ...
        norm_layer = functools.partial(SpectralNorm, affine=False, track_running_stats=False)
    elif norm_type == 'none':
        def norm_layer(x):
            return Identity()
    else:
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, norm_type):
        ...
        if(norm_type=='spectral'):
            conv_block += [norm_layer(nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias)), nn.ReLU(True)]
            if use_dropout:
            ...
            if padding_type == 'reflect':
                ...
                raise NotImplementedError('padding [%s] is not implemented' % padding_type)
            conv_block += [norm_layer(nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias))]
